...
const ModalAddComment = (props) => {
    ...
    return (
        <Modal show={modalAddComments} onHide={handleClose}>
            <Modal.Header>
                <Modal.Title>کامنت</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <textarea placeholder='توضیح خود را بنویسید' value={text} onChange={(e) => setText(e.target.value)} className='w-full h-28 bg-gray-200 rounded-xl p-3 text-sm' />
                <Button onClick={onSubmit} className='mt-2' variant="primary">ذخیره</Button>
            </Modal.Body>
        </Modal>
    )
}
